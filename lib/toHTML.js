...
const main = async () => {
	...
	const headerTags = {
		...
		description: description ? `<meta name="description" content='${description}'>` : undefined,
		icon: icon ? `<link rel="shortcut icon" href="${icon}">` : undefined,
	};
	...
	const finalInjection = stylesheetInjection.toString().replace(/(\${metaTagsInject})/g, Object.keys(headerTags).map(k => headerTags[k] ?? '').filter(Boolean).join('\n'));
	const html = `${finalInjection}
